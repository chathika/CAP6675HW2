...
turtles-own [
  ...
  age              ;; Age of turtle: 1=young, 2=old
]

patches-own [
  priority         ;; Priority of patch: 1=high, 2=low
]
...
to set-hot-spots 
  ...
  ;; Removed the age from this model
...
;  set num-young-green (round (num-green * (%-young / 100)))
;  
;  ;; Distribute the reds based on percent young
;  ask turtles with [ color = red ] [
;    set-young-red 
;  ]
;  
;  ;; Distribute the greens based on percent young
;  ask turtles with [color = green] [
;    set-young-green
;  ]
  
  ;; Set all patches to default priority
  ask patches [ set priority 2 ]
  
  ;; Set hot spots to higher priority
  ask patch 0 0 [ ask patches in-radius hot-spot-radius [ set priority 1 ] ]
  
  update-variables
  reset-ticks
end

;; Color in the areas with higher priority
to show-hot-spots
  clear-all

  ask patch 0 0 [ ask patches in-radius hot-spot-radius [ sprout 1 [ set color red ] ] ]
end

to set-young-red
  if ((count turtles with [color = 17]) != num-young-red) [set color 17]  ;; set to light red
end

to set-young-green
  if ((count turtles with [color = 67]) != num-young-green) [set color 67]  ;; set to light green
end
...
to find-new-spot
  ...
  ifelse tempx != 0 [ setxy tempx tempy ] [
    rt random-float 360
    fd random-float 10
...
to update-turtles
  ask turtles [
    ...
    ;; If old red
    if color = red [set happy? similar-nearby >= (old-%-similar-wanted * total-nearby / 100)]
    ;; If young green
...
to update-globals
  ...
  set num-young-red (count turtles with [color = 17])
  set num-red (num-young-red + (count turtles with [color = red]))
  set num-young-green (count turtles with [color = 67])
  set num-green (num-young-green + (count turtles with [color = green]))
end
